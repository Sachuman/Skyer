...
class Skyland:
    ...
    def __init__(self, canvas):
        ...
        self.canvas.bind_all('<KeyPress-2>', self.open_level2)
        self.paused = False
        self.land = Land(canvas)
        self.AI1 = AI(canvas, 55, 220)
        self.AI2 = AI(canvas, 500, 125)
    ...
    def open_level2(self, event=None):
        # THANKS TO INTERNET FOR SUBPROCESS MODULE
        import subprocess
    ...
    def update(self):
        if not self.paused:
            ...
            self.AI1.update(5, 200, True)
            self.AI2.update(5, 100, True)
            self.land.update()
        ...
        if self.score == 6:
            ...
            self.canvas.delete(self.text)
            self.canvas.unbind_all('<KeyPress-space>')
            self.text = canvas.create_text(150, 370, text='                                              YOU WIN !!! PRESS ALT_L to restart or press 2 for next level!',
                                           font=font.Font(family='Helveca', size=15, weight='bold'))
...
class Land:
    ...
    def make_hill(self, x1, y1, x2, y2, height=100, delta=3):
        midpoint = (x1 + x2) / 2
        hill = self.canvas.create_polygon(x1, y1, midpoint, y1 - height, x2+delta, y1, fill='brown')
        return hill

    def make_cloud(self, x, y):
        cloud = self.canvas.create_oval(x, y, x + 75, y + 40, fill='white', outline='black')
        return cloud

    def cases(self, x1, y1, x2, y2):
        platforms = self.canvas.create_rectangle(x1, y1, x2, y2, fill = 'coral', outline = 'black')
        return platforms
    ...
    def update(self):
        coordsofmovingrec = self.canvas.coords(self.w)
